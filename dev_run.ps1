...
#   .\dev_run.ps1
#
# Cambiá el email de abajo al tuyo antes de correrlo por primera vez —
...
if (Test-Path $venv_activate) {
    ...
    Write-Host "WARN: no encontré venv en .\venv. Si todavía no lo creaste:" -ForegroundColor Yellow
...
# Si querés mandar mails reales (sino caen al outbox data/_outbox/*.eml):
...
Write-Host "    2) Tocar 'Crear una' (link de signup)" -ForegroundColor Yellow
Write-Host "    3) Registrarte con $superadmin_email" -ForegroundColor Yellow
Write-Host "    4) Ya quedás como SUPERADMIN auto-verificado." -ForegroundColor Yellow
Write-Host ""
